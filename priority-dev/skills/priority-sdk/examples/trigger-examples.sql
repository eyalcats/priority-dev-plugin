...
/*=====================================================================
 * HARVESTED FORM-TRIGGER IDIOMS (2026-04-22)
 * Canonical shapes observed across many standard Priority forms.
 * See references/triggers.md § 12 for prose.
 *=====================================================================*/

-- PRE-FORM constant preload. Distilled from: ORDERS + 28 more forms.
-- Query: FORMTRIGTEXT LIKE '%SELECT VALUE INTO %FROM SYSCONST%'
:PORTALONLY = 0;
...
-- Dynamic-zoom EXEC resolver. Distilled from: LOGFILE, ORDERS, WTASKDOCS, DLVTRACKITEMS + 35 more.
-- Query: FORMTRIGTEXT LIKE '%SELECT EXEC INTO %FROM EXEC WHERE ENAME%'
...
-- PRE-FORM typed-zero money/quantity init. Distilled from: ORDERS, FOBORDERS, FOBPROF.
-- Query: FORMTRIGTEXT LIKE '%= 0e-5%'
:PRICE = 0e-5;
:QPRICE = :VAT = :TOTPRICE = :PERCENT = 0e-6;


-- Dynamic-ZOOM one-line include. Distilled from: ORDERS + 21 more forms.
-- Query: FORMTRIGTEXT LIKE '%#INCLUDE FNCTRANS/PRE-FORM%'
#INCLUDE FNCTRANS/PRE-FORM /* dynamic ZOOM */


-- INTERFACE-aware warning gating. Distilled from: ORDERS, AINVOICES, IVPACK,
--  TRANSORDER_D/T/C, WAREHOUSES, PRDISINGLE, DOCUMENTS_D, SERIAL.
-- Query: FORMTRIGTEXT LIKE '%:INTERFACEIGNOREWARNINGS%'
GOTO 1 WHERE :INTERFACEIGNOREWARNINGS = 1;
/* warning-only checks here */
WRNMSG 501 WHERE :$.AMOUNT > 1000000;
LABEL 1;


-- Async fire-and-continue from POST-trigger.
-- Distilled from: ORDERS, DLVTRACKS, ENVCONFIGURATION, ADCSLSCONTROLLER.
-- Query: FORMTRIGTEXT LIKE '%EXECUTE WINACTIV%-P%'
:TMPFILE = '';
SELECT SQL.TMPFILE INTO :TMPFILE FROM DUMMY;
LINK ORDERS TO :TMPFILE;
GOTO 7790 WHERE :RETVAL <= 0;
INSERT INTO ORDERS
SELECT * FROM ORDERS ORIG WHERE ORIG.ORD = 0 + :$.ORD;
...
-- Multi-assign from form buffer. Distilled from: TRANSORDER_P, FOBDOC, YINVOICES,
--  WTASKITEMS, PRDISINGLE, PORDERITEMS + 5 more forms.
-- Query: FORMCLTRIGTEXT LIKE '%SELECT :$.%:$.%INTO :%:%FROM DUMMY%'
:TAX = :AGENT = :AGENT2 = 0;
