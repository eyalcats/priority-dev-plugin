...
### Override tabula.ini for WINRUN

By default, WINRUN uses `C:\Windows\tabula.ini`. To use a different ini file, set the environment variable before running WINRUN:

```cmd
set TABULAINIORIG=xxx.ini
```

where `xxx` is the name of the desired `.ini` file. This is useful in environments with multiple Priority installations.

*(seen in: handbook:Click2Sign@page-312)*

### Message Routing with -err errfile

When using the `-err errfile` option:
- Only **preliminary** messages (license/privilege checks) are written to the error file.
- **Runtime messages** generated by the procedure itself are written to the `ERRMSGS` table, keyed by the USER that ran the WINRUN command, with `TYPE = 'V'`.

```sql
/* Read runtime messages after WINRUN completes */
SELECT MESSAGE FROM ERRMSGS
WHERE USER = SQL.USER AND TYPE = 'V';
```
...
### SOAP / XML Support (25.1+)

Starting with SDK 25.1, the `-content` parameter accepts `application/soap+xml` as the **request** content type. This sets the outgoing `Content-Type` header on the SOAP request. Use `-action` to set the SOAPAction HTTP header required by most SOAP services:
...
Notes:
- **v25.0:** WSCLIENT can receive responses in SOAP/XML format. Parse the XML response body using `XMLPARSE` (with `-all` if needed for large payloads). *(handbook:Release Notes p3)*
...
> **v23.1 note:** In addition to the dedicated `SFTPCLNT` program, `WSCLIENT` gained the ability to upload and download files from external SFTP servers in version 23.1. Both mechanisms were introduced simultaneously. See the WSCLIENT chapter of the handbook for the exact parameter syntax for WSCLIENT SFTP mode; this section covers the `SFTPCLNT` configuration-based approach.

*(seen in: handbook:Release Notes@page-4)*
