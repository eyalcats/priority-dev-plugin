...
## Handbook scouting is a different agent

`/handbook-scan` does NOT spawn this agent. It spawns `priority-dev:handbook-scout` (defined in `plugin/agents/handbook-scout.md`), which has `Read`/`Grep`/`Bash`/`Write` and no Priority MCP tools. Handbook scouting reads files; it never queries the live server. If a future caller passes you `"mode": "handbook-scout"`, decline — the file tools you would need are not in your inventory.
